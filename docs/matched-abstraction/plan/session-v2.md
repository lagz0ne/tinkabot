...
- Resolved 2026-06-12 (Slice 6 wrapper language/placement): this Plan originally pinned a Bun wrapper at repo-root `apps/wrapper`; implementation landed a Go wrapper at `substrate/go/apps/wrapper`. The drift was escalated rather than silently accepted, and the user decided: Go, because the project consolidated on Go for everything embedded-NATS-facing. The Approach's Purpose framing was generalized from "by Bun" to a language-neutral wrapper process; none of the eight invariants are touched. Slice 6 text above is amended accordingly.
