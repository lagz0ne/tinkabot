...
| Go substrate | NATS lifecycle, stores, auth rendering, connection policy, process boundary, gateway boundary, activation ledger storage, artifact gateway substrate, attribution, and future sandbox boundary |
| Browser edge | Session bootstrap, browser credential lease, worker principal, credential mint/revoke, artifact serving, cache/CSP/frame sandbox policy, and browser-to-NATS handoff |
| Frontend substrate | Trusted shell, dedicated worker, generated-content isolation, sub-app principals, typed intent emission, materialized rendering, and brokered cross-sub-app communication |
...
| Browser edge | Invalid session, credential mint failure, credential revoke failure, artifact integrity failure, CSP/frame policy failure |
| Frontend substrate | Denied IPC, raw-NATS vocabulary attempt, stale frame revision, sub-app principal mismatch, broker denial |
...
| Browser edge | Worker principal is scoped, generated content cannot see credentials or subjects, artifact revision and CSP/frame policy are enforced |
