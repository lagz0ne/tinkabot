...
- Confirmed quality-v1 candidates against pinned deps (nats-server v2.14.2, nats.go v1.52.0): dynamic lease auth via `server.Options.CustomClientAuthentication` (per-connection Go auth decision over a live lease store; closes the deferred live-auth-reload item, existing-connection revocation needs explicit disconnect), and portless embedded NATS via `server.Options.DontListen` + `Server.InProcessConn()` + `nats.InProcessServer(...)` (in-process default; a loopback listener stays opt-in because the `nats` CLI and outside-in proofs need a socket).
