...
- RED: `go test ./embednats -run 'TestTrustedWrapperAuthority' -count=1` FAIL — all 4 subtests (OverbroadMint, SelfDeclaredTrust, SteerAfterRevoke, FakeStatusImpersonation) failed: no subject-breadth check in `MintUser`, `MintTrustedWrapper`/`ApplySteerAfterRevoke` stubs returned "not implemented", `StartFrameMediator` called `addSessionUser` (incompatible with operator mode). Pre-existing tests stayed green.
- GREEN: Subject-breadth check in `MintUser` (`isSessionSubtreeWildcard` denies `tb.session.`-prefixed wildcard grants, `OverbroadMint` typed Kind constant). `MintTrustedWrapper` issues leaf-scoped JWT (publish ingest only, subscribe steer + `_INBOX.>`). `IsRevoked` + `ApplySteerAfterRevoke` re-check revocation at apply time returning `(false, nil)` if revoked. `StartFrameMediator` operator-mode support via `mintedConn` using `MintUser` for the mediator principal. All 4 subtests green + 9 outside-in proof sub-tests green over real embedded NATS in operator/JWT mode.
- Verified (full battery, 2026-06-11): `bun run test` 100 pass/492 expects (18 files), `test:e2e` 1 pass, `typecheck`, `build`, `pack:dry`, `schema:parity` (25 contract tests, 7 Go packages ok), `go test ./... -count=1` (7 packages ok), `release:evidence` (16 milestones / 11 spine steps / 5 gate results), `validate:layers`, `test:layers` (10 tests OK), all five `gate:*` (gate:coverage: all 7 layers meet thresholds), `git diff --check` — all pass. Gates real-nats, parallel-safety, coverage, security, be-lazy, no-slop all pass. Evidence in `docs/matched-abstraction/task/trusted-wrapper-authority.md` (status complete).
