...
Matched-abstraction skill extraction — DONE (2026-06-12): skill is now portable/self-contained (branch-question posture inlined; no C3/grill-me/tasks startup ritual; inline and persistence modes first-class), layer validation moved from Python to Bun/TypeScript at `.codex/skills/matched-abstraction-thinking/scripts/validate-layers.ts`, `package.json` now routes `test:layers`/`validate:layers` through Bun, and the old Python validator/test files were removed. RED was `bun test tests/validate-layers.test.ts` failing on the missing TS validator, then failing on stale skill dependency language. Verified: `bun run test:layers` 12/12, `bun test tests/*.test.ts` 56/56, `bun run test` 118/118, `bun run typecheck` pass, tracked touched-file `git diff --check` pass, and new-file trailing-whitespace scan pass. Caveat: `bun run validate:layers` reaches the new validator but fails on unrelated untracked `docs/matched-abstraction/task/bundle-transform.md` because its verification evidence is still promissory.
