...
Current slice: `session-runtime-subsystem` — DONE (session-v2 slice 2/7).

RED-GREEN-TDD result:

- RED: `go test ./embednats -run 'TestSessionLiveness' -count=1` FAIL — `TestSessionLivenessLeaseExpiry` failed (OpenLivenessStore not implemented); `go test ./embednats -run 'TestSessionRuntime' -count=1` FAIL — all four sub-tests failed (StartSessionRuntime, ReconcileOrphanedSession, OpenLivenessStore not implemented). Pre-existing tests stayed green.
- GREEN: `SessionRuntime` supervised process, `LivenessStore` JetStream KV bucket with bucket-level TTL (jetstream package, MemoryStorage for sub-second expiry), `OpenLivenessStore`/`ClaimLiveness`/`IsAlive`/`HeartbeatLiveness`, `StartSessionRuntime`/`ReconcileOrphanedSession`, `ReadTerminalRecord`/`SessionRecord`, per-session least-authority credential, denied-neighbor output-parsed proof. All four sub-tests and both liveness tests green over real embedded NATS.
- Verified (full battery, 2026-06-11): `bun run test` 100 pass/492 expects (18 files), `test:e2e` 1 pass, `typecheck`, `build`, `pack:dry`, `schema:parity` (25 contract tests, 7 Go packages ok), `go test ./... -count=1` (7 packages ok), `release:evidence` (16 milestones / 11 spine steps / 5 gate results), `validate:layers`, `test:layers` (10 tests OK), all five `gate:*` (coverage: all layers above thresholds, embednats 77.9%>=72%), `git diff --check` — all pass. Gates real-nats, parallel-safety, coverage, security, be-lazy, no-slop all pass. Evidence in `docs/matched-abstraction/task/session-runtime-subsystem.md` (status complete).
